document.addEventListener('DOMContentLoaded', function () {
    ...
    function mostrarTabelaSimplex(tabela, titulo, linhaPivo = -1, colunaPivo = -1, isFinalIteration = false) {
        ...
        let tabelaHTML = `<h2 class="tabela-titulo">${titulo}</h2><table class="table">`;
        // Cabeçalho da tabela
        tabelaHTML += '<tr><th></th>';
        for (let i = 0; i < numVariaveis; i++) {
            tabelaHTML += `<th>x${i + 1}</th>`;
        ...
        tabelaHTML += `<th>RHS</th></tr></thead><tbody>`; // Cabeçalho para a coluna RHS
        ...
        tabelaContainer.insertAdjacentHTML('beforeend', tabelaHTML); // Adiciona a nova tabela sem limpar as anteriores
    }
