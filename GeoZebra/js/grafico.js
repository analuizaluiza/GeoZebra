document.addEventListener('DOMContentLoaded', function () {
    ...
    // Se um ponto ótimo inteiro foi encontrado, adicioná-lo ao gráfico
    if (melhorPontoInteiro) {
        traces.push({
            type: 'scatter',
            mode: 'markers',
            x: [melhorPontoInteiro.x],
            y: [melhorPontoInteiro.y],
            marker: { color: 'blue', size: 10 },
            text: `Solução Inteira = (${melhorPontoInteiro.x}, ${melhorPontoInteiro.y}), Z = ${melhorValor.toFixed(2)}`,
            name: `Solução Ótima Inteira = ${melhorValor.toFixed(2)}`,
        });
    } else {
        console.log("Nenhum ponto inteiro viável encontrado dentro da região.");
    }
}
    ...
    const layout = {
        title: {
            text: 'Resolução Gráfica do Problema de Otimização Linear',
    ...
    function gerarIsocurva(coefObjetivo, z, xMax, yMax) {
        const trace = {
            ...
            mode: 'lines',
            name: `Z = ${z.toFixed(2)}`,
            line: { color: getRandomColor(), dash: 'dash' },
            x: [],
            y: []
        };
